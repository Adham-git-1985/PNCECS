...
def is_safe_url(target: str) -> bool:
    ...
    # Absolute URL: allow only same host (ignore port)
    if parsed.scheme or parsed.netloc:
        ref = urlparse(request.host_url)

        ref_netloc = (CANONICAL_HOST or ref.netloc).lower()
        parsed_netloc = (parsed.netloc or "").lower()

        ref_host = ref_netloc.split(":")[0]
        parsed_host = parsed_netloc.split(":")[0]

        return parsed.scheme in ("http", "https") and parsed_host == ref_host
...
with app.app_context():
    try:
        ...
        admin_password = config.get("admin_password", "Adm!n@123")
...
def request_entity_too_large(error):
    ...
    return redirect_back('index') # إعادة التوجيه إلى الصفحة السابقة
