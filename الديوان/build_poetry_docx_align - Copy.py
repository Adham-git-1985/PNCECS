# -*- coding: utf-8 -*-
"""
build_poetry_docx_align.py (nowrap support)
------------------------------------------
- يدعم A4/A5/Letter + هوامش + أبعاد تلقائية + فجوة عمود/إزاحة + محاذاة لكل شطر + RTL + تخطيط صفّين.
- جديد: --nowrap لمنع التفاف الشطر داخل الخلية (يثبّت الشطر كسطر واحد).

استخدام مقترح (A5، صفّان لكل بيت، فجوة 0.6سم، منع التفاف):
    python build_poetry_docx_align.py --in poems.txt --out ديواني.docx --page A5 --margin 1 \
      --layout two-rows --auto-widths --gap 0.6 --gap-mode column \
      --right-align right --second-align left --font "Traditional Arabic" --size 18 --nowrap
"""
...
def disable_cell_wrap(cell):
    """مَنْع التفاف النص داخل الخلية (سطر واحد مهما طال)."""
    tcPr = cell._tc.get_or_add_tcPr()
...
def main():
    parser = argparse.ArgumentParser(description="Arabic poetry layout with A5/A4, gap, auto-widths, and nowrap option.")
    ...
    parser.add_argument("--second-align", choices=["right", "left", "justify", "distribute"], default="left",
