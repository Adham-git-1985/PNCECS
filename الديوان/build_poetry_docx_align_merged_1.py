...
---------------------------------
نسخة مبنية على سكربتك الشغّال، مع الحفاظ على كل السلوك السابق كما هو،
وإضافة ميزتين اختياريتين تعملان فقط عند طلبهما:
  ...
  2) --second-shift-pct N  : ترك فراغ أبيض يسار الشطر الثاني بنسبة N%% من عرض عمود الشطر الأول w1.
     - تعمل فقط مع --layout two-rows.
     - تبقى المحاذاة للشطر الثاني كما تحددها (--second-align right/left/justify/distribute).

الاستخدام المقترح (A5 + صفّان + gap بسيط + دمج + إزاحة 75%%):
...
def set_paragraph_rtl(paragraph, enable=True):
    ...
    bidi = pPr.find(_qn('w:bidi'))
    if bidi is None:
        bidi = OxmlElement('w:bidi')
        pPr.append(bidi)
    bidi.set(_qn('w:val'), '1' if enable else '0')
...
def build_poetry_docx_align(
    ...
    merge_second=False, second_shift_pct=0.0,
):
    """
    يحافظ على كل السلوك الافتراضي السابق.
    لو merge_second=False فالسلوك مطابق تمامًا للنسخة القديمة.
    second_shift_pct يُطبّق فقط عندما layout='two-rows' و merge_second=True.
    """
    # تحقّق من القيم
    ...
    # مقدار الإزاحة كنسبة من w1 (يُستخدم فقط مع merge_second + two-rows)
    shift_cm = 0.0
    if layout == 'two-rows' and merge_second:
        shift_cm = max(0.0, (second_shift_pct or 0.0) * 0.01 * w1)

    if layout == "row":
        # === نفس السلوك السابق تمامًا ===
        if gap_mode == "column":
        ...
        if gap_mode == "column":
            ...
            for i, (right_hemistich, left_hemistich) in enumerate(poems):
                ...
                if merge_second:
                    ...
                    # محاذاة الفقرة حسب second-align (مع الحفاظ على RTL)
                    p2 = merged.paragraphs[0]; p2.alignment = align_second; set_paragraph_rtl(p2, rtl)
                    # فراغ أبيض يسار الخلية (END في RTL)
                    set_cell_margins(merged, top=0, start=0, bottom=0, end=cm_to_twips(shift_cm))
            ...
            for i, (right_hemistich, left_hemistich) in enumerate(poems):
                ...
                if merge_second:
                    ...
                    p2 = merged.paragraphs[0]; p2.alignment = align_second; set_paragraph_rtl(p2, rtl)
                    # فراغ أبيض يسار الخلية = gap_cm + shift_cm
                    total_left_white_cm = max(0.0, gap_cm) + max(0.0, shift_cm)
                    set_cell_margins(merged, top=0, start=0, bottom=0, end=cm_to_twips(total_left_white_cm))
                    if nowrap: disable_cell_wrap(merged)
                    run2 = p2.add_run(left_hemistich); run2.font.name = font_name; run2.font.size = Pt(font_size_pt)
                else:
...
def main():
    parser = argparse.ArgumentParser(description="Arabic poetry layout with A5/A4, gap, auto-widths, nowrap, and optional merging/shift.")
    ...
    if args.infile:
        ...
        poems = [
            ("بوركت يا بلدي فيك الوفاء مَثَلُ", "يا شامةً تُوِّجَتْ وطْني فلسطيننا"),
